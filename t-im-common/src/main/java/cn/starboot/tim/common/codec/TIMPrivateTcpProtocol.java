...
public abstract class TIMPrivateTcpProtocol implements AioHandler {
    ...
    public Packet decode(MemoryUnit readBuffer, ChannelContext channelContext) throws AioDecoderException {
        ...
        // 获取命令码
        CommandType command = CommandType.getCommandTypeByCode(commandType);
        // 判断是否为异常命令
        if (command == null) {
            // 用户发送未知命令码
    ...
    public void encode(Packet packet, ChannelContext channelContext) throws AioEncoderException {
        ...
        if (packet instanceof ImPacket) {
            ...
            // 消息命令码
            CommandType commandType = imPacket.getCommandType();
            // 消息体
            ...
            // 写入命令码
            writeBuffer.writeInt(commandType.getCode());
            // 写入消息体长度
